...
Once an ongoing blue-green upgrade is canceled, the rollback can be initiated using the [`az aks nodepool rollback`](/cli/azure/aks/nodepool#az-aks-nodepool-rollback) command. <br />
The rollback is only available during the final soak period as described in the [finalSoakDurationInMinutes](/azure/aks/blue-green-node-pool-upgrade#customize-blue-green-upgrade-properties) property. <br />
